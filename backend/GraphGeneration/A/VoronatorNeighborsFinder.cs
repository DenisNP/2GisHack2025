...
public class VoronatorNeighborsFinder
{
    ...
    public NeighborhoodResult FindNeighborsWithEdges(EdgeFilter edgeFilter, Vector2 point)
    {
        ...
        try
        {
            ...
            foreach (var neighborIndex in neighborIndices)
            {
                // if (neighborIndex >= 0 && neighborIndex < _allPoints.Count)
                {
                    ...
                    if (edgeFilter.Skip(point, neighborPoint))
                    {
                        continue;
                    }
            ...
            FindEdgesBetweenNeighbors(edgeFilter, point, neighborSet, edgeSet);
    ...
    private void FindEdgesBetweenNeighbors(EdgeFilter edgeFilter, Vector2 centerPoint, HashSet<Vector2> neighbors, HashSet<VoronatorFinderEdge> edges)
    {
        ...
        for (var i = 0; i < neighborList.Count; i++)
        {
            for (var j = i + 1; j < neighborList.Count; j++)
            {
                ...
                if (edgeFilter.Skip(pointA, pointB))
                {
                    continue;
                }
