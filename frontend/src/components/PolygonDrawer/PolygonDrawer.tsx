...
export function PolygonDrawer({ color = '#007acc', label = 'Тип', polygons, onPolygonsChanged, position }: PolygonDrawerProps) {
        ...
        const onClick = (e: any) => {
            ...
            if (currentPoints.length >= 1) {
                try {
                    ...
                    const pPx = mapglInstance.project(point);
                    const fPx = mapglInstance.project(first);
                    console.log({...pPx})
                    const dx = pPx[0] - fPx[0];
                    const dy = pPx[1] - fPx[1];
                    const dist = Math.sqrt(dx * dx + dy * dy);
                    if (dist < 12) {
                        if (currentPoints.length >= 3) {
                            const coords = [closeRing(currentPoints)];
                            const id = String(nextPolygonIndexRef.current++);
                                    // logical polygon entry only; map instances are synced from effectivePolygons
                        ...
                        if (tempLineRef.current) {
                            try { tempLineRef.current.destroy(); } catch (err) {}
                        ...
                        if (firstPointMarkerRef.current) {
                            ...
                            try { firstPointMarkerRef.current.destroy(); } catch (err) {}
    ...
    }, [mapglInstance, mapgl, isDrawing, currentPoints, color, colorDerived, colorRgb, polygons, onPolygonsChanged, effectivePolygons]);
    ...
    }, [currentPoints, mapglInstance, mapgl, colorDerived, colorRgb, polygons, effectivePolygons, onPolygonsChanged]);
